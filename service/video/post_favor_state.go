...
func (p *PostFavorStateFlow) PlusOperation() error {
	//视频点赞数目+1
	err := repository.NewVideoDAO().PlusOneFavorByUserIdAndVideoId(p.userId, p.videoId)
	if err != nil {
		return errors.New("不要重复点赞")
	}
	//对应的用户是否点赞的映射状态更新
	repository.UpdateVideoFavorState(p.userId, p.videoId, true)
	return nil
}
...
func (p *PostFavorStateFlow) MinusOperation() error {
	//视频点赞数目-1
	err := repository.NewVideoDAO().MinusOneFavorByUserIdAndVideoId(p.userId, p.videoId)
	if err != nil {
		return errors.New("点赞数目已经为0")
	}
	//对应的用户是否点赞的映射状态更新
	repository.UpdateVideoFavorState(p.userId, p.videoId, false)
	return nil
}

func (p *PostFavorStateFlow) checkNum() error {
	if !repository.InitUserDao().IsExistUserId(p.userId) {
